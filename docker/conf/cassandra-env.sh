...
#
echo $JVM_OPTS | grep -q LOCAL_JMX
if [ "x$LOCAL_JMX" = "x" ] || [ $? -eq 0 ]; then
    LOCAL_JMX=yes
fi

echo $JVM_OPTS | grep -q REMOTE_JMX
if [ $? -eq 0 ]; then
    REMOTE_JMX=yes
fi
...
REMOTE_JMX_AUTH=yes
echo $JVM_OPTS | grep -q REMOTE_JMX_NO_AUTH
if [ $? -eq 0 ]; then
    REMOTE_JMX_AUTH=no
fi

echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
echo "LOCAL_JMX=$LOCAL_JMX; REMOTE_JMX=$REMOTE_JMX; REMOTE_JMX_AUTH=$REMOTE_JMX_AUTH;"
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
...
if [ "$REMOTE_JMX" = "yes" ]; then
  ...
  JVM_OPTS="$JVM_OPTS -Dcom.sun.management.jmxremote.authenticate=$REMOTE_JMX_AUTH"
